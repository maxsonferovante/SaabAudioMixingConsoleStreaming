...
async fn test_end_to_end_udp_audio_streaming() {
    ...
    let ring_buffer = Arc::new(HeapRb::<f32>::new(4096));

    // Start Receiver
    let receiver = UdpAudioReceiver::new();
    receiver
        .start(receiver_addr, Arc::clone(&ring_buffer))
    ...
    // Send known test signal (4 distinct samples)
    ...
    streamer
        .stream_audio(&buffer, 1, 1000)
        .expect("stream audio packet");

    // Wait for packet arrival
    tokio::time::sleep(Duration::from_millis(100)).await;

    // Verify samples received in ring buffer
    let (_, mut consumer) = ring_buffer.split();
